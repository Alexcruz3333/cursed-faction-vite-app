...
export default function App() {
  ...
  const downloadFeed = () => { try { const blob = new Blob([JSON.stringify(feed, null, 2)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'cursed-faction-activity-' + Date.now() + '.json'; a.click(); URL.revokeObjectURL(url); } catch { /* noop */ } }
  ...
  const disconnect = () => { try { const addr = account; localStorage.removeItem(LS_ACCOUNT); localStorage.removeItem(LS_PROVIDER); setAccount(''); setChainId(''); setBaseEth(''); setTokenInfo({ symbol: '', decimals: 18, balance: '' }); setMessage('Disconnected'); pushEvent('wallet','Disconnected', addr || '') } catch { /* noop */ } }
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:24, alignItems:'start', marginTop:16}}>
        <div>
          ...
          <section style={{marginTop: 24}}>
            <h2>NFT Mint</h2>
            <div style={{display:'grid', gap:8, gridTemplateColumns:'1 1 1'}}>
